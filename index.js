...
app.post("/register", async (req, res) => {
  try {
    ...
    const collection = db.collection("adminReg");
...
app.post("/sendpasswordlink", async (req, res) => {
  ...
  try {
    ...
    if (userfind) {
      ...
      const setrandomnum = await collection.findOneAndUpdate({ email: req.body.email }, {
        $set: {
          rnum: randomnum
        }
      });
...
app.post("/verify-user/:id/:randomnum", async (req, res) => {
  try {
    ...
    const db = connection.db("registrations");
    const collection = db.collection("adminReg");
    const userfind = await collection.findOne({ _id: mongodb.ObjectId(req.params.id) });
...
app.put("/password-update/:id", async (req, res) => {
  try {
    ...
    const users = await collection.findOneAndUpdate({ _id: mongodb.ObjectId(req.params.id) }, {
      $set: {
        password: hash1,
